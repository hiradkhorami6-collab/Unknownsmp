...
form.addEventListener("submit", async (event) => {
    ...
    try {
        ...
        // Lock this Telegram ID on this browser only after Discord confirms success.
        markSubmitted(telegram);
        form.reset();
        showMessage("درخواست شما با موفقیت ارسال شد. بعد از تایید ادمین، گپ تلگرام برایتان ارسال می‌شود. ✓", "success");
    } catch (error) {
        console.error("UNKNOWN SMP webhook error:", error);

        if (error instanceof TypeError) {
            showMessage("مرورگر نتوانست به Discord متصل شود. اگر روی GitHub Pages هستید، برای جلوگیری از مشکل CORS بهتر است Webhook از طریق Worker ارسال شود.", "error");
        } else {
            showMessage("ارسال به Discord ناموفق بود. Webhook و کانال Discord را بررسی کنید.", "error");
        }
    } finally {
