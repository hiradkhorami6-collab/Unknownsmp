...
const COUNTDOWN_KEY = "unknown_smp_whitelist_close_time";
...
form.addEventListener("submit", async (event) => {
    ...
    try {
        // Discord webhooks accept cross-origin POST requests. `wait=true`
        // lets us verify that Discord actually accepted the message.
...
function getWhitelistCloseTime() {
    try {
...
function updateJoinCountdown() {
    ...
    joinCountdown.textContent =
        `${days} روز ${String(hours).padStart(2, "0")}:${String(minutes).padStart(2, "0")}:${String(seconds).padStart(2, "0")}`;
}
